...
export function registerStatusCommand(program: Command): void {
  program
    .command('status')
    .description('Show LocalhostFix config, integration state, and the last inspection result')
    .action(() => {
      ...
      console.log('');
      console.log(pc.bold('  LocalhostFix status'));
      console.log(`  Project root : ${project.root}`);
      console.log(`  Config       : ${source ? path.relative(process.cwd(), source) : 'none — run `localhostfix setup`'}`);
      ...
      const latestReport = path.join(localhostfixDir(project.root), 'latest', 'report.json');
