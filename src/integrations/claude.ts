 ...
 *
 * - Skill:  .claude/skills/agentview/SKILL.md  (auto-discovered by Claude Code)
 ...
 *
 * Everything AgentView writes is additive: existing settings keys and existing
 * hooks are preserved; we only append our own entries, identified by the
 * AGENTVIEW_HOOK marker so they can be found and removed cleanly.
 */

export const SKILL_DIR = path.join('.claude', 'skills', 'agentview');

/**
 * Hooks go to settings.local.json by default: it is personal and
 * git-ignored, so installing AgentView never imposes a browser-launching
...
export const AGENTVIEW_HOOK_MARKER = 'agentview-hook';

export interface ClaudeStatus {
  skillInstalled: boolean;
  hookInstalled: boolean;
  /** Which settings file currently carries the AgentView hooks. */
...
export function claudeIntegrationStatus(projectRoot: string): ClaudeStatus {
  ...
  for (const file of [LOCAL_SETTINGS_FILE, SHARED_SETTINGS_FILE]) {
    ...
    try {
      if (fs.readFileSync(settingsPath, 'utf8').includes(AGENTVIEW_HOOK_MARKER)) {
